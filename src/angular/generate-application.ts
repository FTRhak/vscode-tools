...
import { NameElement, PrefixElement, SkipTestsElement, StandaloneElement, StylesElement, TypeElement } from '../form_elements';
import { PathElement } from '../form_elements/path.element';

export function angularCommandGenerateApplication(context: vscode.ExtensionContext): vscode.Disposable[] {
                    ...
                    message => {
                        if (message.command === 'angular-create-application') {
                            ...
                            if (message.type === 'ng') {
                                ...
                                command = `nx g @nx/angular:application ${path}/${in_folder?(name + '/'):''}`
