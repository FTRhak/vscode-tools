...
import { InFolderElement, NameElement, SkipImportModuleElement, SkipTestsElement, StandaloneElement, SufixElement, TypeElement } from '../form_elements';
import { PathElement } from '../form_elements/path.element';

export function angularCommandGeneratePipe(context: vscode.ExtensionContext): vscode.Disposable[] {
                    ...
                    message => {
                        if (message.command === 'angular-create-pipe') {
                            ...
                            if (message.type === 'ng') {
                                ...
                                command = `nx g @nx/angular:pipe ${path}/${in_folder?(name + '/'):''}${name}${sufix?'.pipe':''}`
